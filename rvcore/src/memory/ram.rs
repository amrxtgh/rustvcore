...
impl RAM {
    ...
    pub fn load8(&self, addr: u32) -> Result<u8, Exception> {
        ...
        if addr as u64 >= DRAM_SIZE { return Err(Exception::LoadAccessFault); }
        Ok(self.data[addr as usize])
    }
    pub fn load16(&self, addr: u32) -> Result<u16, Exception> {
        /* Alignment check if the address is divisible by 2*/
        if addr % 2 != 0 { return Err(Exception::LoadAddressMisaligned); }
        if addr as u64 + 1 >= DRAM_SIZE { return Err(Exception::LoadAccessFault); }
    ...
    pub fn store8(&mut self, addr: u32, value: u8) -> Result<(), Exception> {
        if addr as u64 >= DRAM_SIZE { return Err(Exception::StoreAccessFault); }
        let index = addr as usize;
        self.data[index] = value;
        Ok(())
    }
    pub fn store16(&mut self, addr: u32, value: u16) -> Result<(), Exception> {
        if addr % 2 != 0 { return Err(Exception::StoreAddressMisaligned); }
        if addr as u64 + 1 >= DRAM_SIZE { return Err(Exception::StoreAccessFault); }
        let index = addr as usize;
    ...
    pub fn store32(&mut self, addr: u32, value: u32) -> Result<(), Exception> {
        if addr % 4 != 0 { return Err(Exception::StoreAddressMisaligned); }
        if addr as u64 + 3 >= DRAM_SIZE { return Err(Exception::StoreAccessFault); }
        let index = addr as usize;
